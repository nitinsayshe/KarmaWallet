...
export const register = async ({ password, name, token, promo, visitorId, isAutoGenerated = false }: IRegisterUserData) => {
  ...
  if (isAutoGenerated) {
    const existingVisitor = await VisitorModel.findOne({ _id: visitorId });
    const { sscid, sscidCreatedOn, xTypeParam, trackingId } = existingVisitor.integrations.shareASale;
    ...
    await openBrowserAndAddShareASaleCode(sscid, xTypeParam, trackingId, sscidCreatedOn);
  }
  ...
  const { sscid, sscidCreatedOn, xTypeParam, trackingId } = visitor.integrations.shareASale;
  ...
  if (!!existingUser?._id && existingUser?.integrations?.marqeta?.status !== currentMarqetaUserState?.status) {
    ...
    if (currentMarqetaUserState.status === IMarqetaUserStatus.ACTIVE) {
      ...
      if (!existingKarmaWelcomeNotification) {
        ...
        // Call the Pupeter script to append Shareasale tracking pixel to a new instance of chrome in headless mode
        const { sscid, xTypeParam, trackingId, sscidCreatedOn } = existingUser.integrations.shareasale;
        await openBrowserAndAddShareASaleCode(sscid, xTypeParam, trackingId, sscidCreatedOn);
      }
    }
