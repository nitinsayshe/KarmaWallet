...
export const mapUserTransactionsFromPlaid = async (userId: string[], daysInPast: number) => {
  ...
  if (!userCards.length) throw new Error('user cards not found');

  const mapper = new PlaidMapper();
  await mapper.mapTransactionsFromPlaid(userCards.map(c => c.integrations.plaid.accessToken), daysInPast);
...
export const mapUsersTransactionsFromPlaid = async (userIds: string[], daysInPast: number) => {
  if (daysInPast > 730) throw new Error('invalid daysInPast. cannot be greater than 730 days (2 years).');

  const userCards = await CardModel.find({ 'integrations.plaid': { $ne: null }, status: 'linked' });
