...
export const register = async ({ password, name, token, promo, visitorId, isAutoGenerated = false }: IRegisterUserData) => {
  ...
  try {
    ...
    if (!!groupCode) {
      ...
      // if auto created user, we need to add them to group, this is usually done by frontend when traditional path to create user
      if (!!isAutoGenerated) {
        const mockRequest = ({
          requestor: {},
          authKey: '',
          body: { code: groupCode, email: newUser.emails[0].email, userId: newUser._id.toString() },
        } as any);
    ...
    if (!!isAutoGenerated) {
      initiateChangePasswordEmail(newUser, email);
    }
    return responseInfo;
