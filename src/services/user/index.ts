...
export const register = async ({ password, name, token, promo, visitorId, isAutoGenerated = false }: IRegisterUserData) => {
  ...
  try {
    ...
    if (!!isAutoGenerated) {
      const joinGroupSuccess = await updateActiveCampaignDataAndJoinGroupForApplicant(newUser, urlParams);
      if (!joinGroupSuccess) throw new CustomError('Error joining group', ErrorTypes.SERVER);
      if (!!isAutoGenerated) await initiateChangePasswordEmail(newUser, email);
    } else {
