...
import { updateKardMerchants, updateKardMerchantRates, matchKardCompanies, narrowDownMatchesByScore, associateKardMatches, RestrictiveMatchScoreThreshold } from '../services/scripts/kardMerchantUpdate';
...
export const matchMerchants = async () => {
  ...
  const merchantNames = (await Promise.all(incomingMerchantNames.map(async (name) => {
    try {
      ...
      if (!company?._id) {
        console.log(`No company wih name: ${name} found`);
        console.log('skipping for now...');
        return null;
      }

      const merchant = await MerchantModel.findOne({ name });
      if (!merchant?._id) {
        return name;
      }
      return null;
  ...
  console.log(`Found ${merchants.length} matches`);
  ...
  const narrowedMatches = await narrowDownMatchesByScore({ inputMatches: initalMatchingResults?.matches, minMatchScore: RestrictiveMatchScoreThreshold });
  ...
  await Promise.all(incomingMerchants.map(async (incomingMerchant) => {
    const matchedAllMerchantNames = incomingMerchant?.names?.every((name) => matchingResults?.matches?.find((m) => m.name === name));
    if (incomingMerchant) {
  ...
  // TODO: update karma collective status on companies
...
export const exec = async (data: IJobData) => {
  switch (data?.syncType) {
    ...
    case KardSyncType.UpdateMerchantRates:
      ...
      break;
    case KardSyncType.MatchMerchants:
      await matchMerchants();
      break;
    default:
