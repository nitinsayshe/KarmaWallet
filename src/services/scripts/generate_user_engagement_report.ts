    ...
    await Promise.all(
      users.map(async (user) => {
        ...
        if (!!cards && cards.length > 0) {
          ...
          if (cards.length > 1) console.log('cards', JSON.stringify(cards, null, 2));
        }
...
export const generateUserEngagementReportNoLinkedCards = async (): Promise<
...
> => {
  try {
    ...
    const unlinkedCardUsers = await CardModel.aggregate()
      ...
      .group({ _id: '$userId' });
    const userIds = unlinkedCardUsers.map((id) => id._id);
    ...
    await Promise.all(
      userIds.map(async (id) => {
        ...
        if (!!linkedCards && linkedCards.length > 0) {
          ...
          if (linkedCards.length > 1) console.log('cards', JSON.stringify(linkedCards, null, 2));
        }
